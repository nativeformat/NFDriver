...
NFDriverFileMP3Implementation::NFDriverFileMP3Implementation(void *clientdata,
...
void NFDriverFileMP3Implementation::run(NFDriverFileMP3Implementation *driver) {
    ...
    decltype(&lame_set_VBR) lame_set_VBR_dynamic = (decltype(&lame_set_VBR))dlsym(lame_handle, "lame_set_VBR");
    decltype(&lame_init_params) lame_init_params_dynamic = (decltype(&lame_init_params))dlsym(lame_handle, "lame_init_params");
    decltype(&lame_encode_buffer_interleaved_ieee_float) lame_encode_buffer_interleaved_ieee_float_dynamic = (decltype(&lame_encode_buffer_interleaved_ieee_float))dlsym(lame_handle,
                                                                                                                                                                         "lame_encode_buffer_interleaved_ieee_float");
    decltype(&lame_encode_flush) lame_encode_flush_dynamic = (decltype(&lame_encode_flush))dlsym(lame_handle,
                                                                                                 "lame_encode_flush");
  ...
  if (fhandle == nullptr) {
    ...
    do {
        ...
        const size_t num_frames = (size_t)driver->_render_callback(driver->_clientdata, buffer, NF_DRIVER_SAMPLE_BLOCK_SIZE);
        if (num_frames < 1) {
          driver->_stutter_callback(driver->_clientdata);
        } else {
            const auto write = lame_encode_buffer_interleaved_ieee_float_dynamic(lame,
                                                                                 buffer,
                                                                                 num_frames,
                                                                                 mp3_buffer,
                                                                                 sizeof(mp3_buffer));
