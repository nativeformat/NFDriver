...
static OSStatus
defaultDeviceChangedCallback(AudioObjectID inObjectID, UInt32 inNumberAddresses,
                             const AudioObjectPropertyAddress inAddresses[],
                             void *inClientData) {
  NFSoundCardDriverInternals *internals =
      (NFSoundCardDriverInternals *)inClientData;
  dispatch_async(dispatch_get_main_queue(), ^{
    recreateAudioUnit(internals);
    startAudioUnitIfNeeded(internals);
  });
...
NFSoundCardDriver::NFSoundCardDriver(
  ...
  AudioObjectSetPropertyData(kAudioObjectSystemObject, &address, 0, NULL,
                             sizeof(CFRunLoopRef), &runLoop);
  ...
  AudioObjectAddPropertyListener(kAudioObjectSystemObject, &address,
                                 defaultDeviceChangedCallback, internals);
}
...
static void recreateAudioUnit(NFSoundCardDriverInternals *internals) {
  if (dispatch_get_specific(mainQueueKey) ==
    ...
    if (AudioObjectGetPropertyData(kAudioObjectSystemObject, &address, 0, NULL,
      ...
      destroyAudioUnit(&unit);
      internals->errorCallback(internals->clientdata,
                               "Can't get the default audio output device.", 0);
      return;
    }
    if (AudioUnitSetProperty(unit, kAudioOutputUnitProperty_CurrentDevice,
      ...
      internals->errorCallback(
          ...
          "Can't set the current output device for the audio unit.", 0);
      return;
    }
    ...
    // though.
    UInt32 numFrames = (UInt32)NFDriverAdapter::getOptimalNumberOfFrames(
        (int)format.mSampleRate);
    address = {kAudioDevicePropertyBufferFrameSize,
               kAudioObjectPropertyScopeGlobal,
               kAudioObjectPropertyElementMaster};
    AudioObjectSetPropertyData(device, &address, 0, NULL, sizeof(numFrames),
                               &numFrames);
