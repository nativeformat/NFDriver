...
// rate.
static void streamFormatChangedCallback(
    void *inRefCon, AudioUnit inUnit, AudioUnitPropertyID inID, AudioUnitScope inScope, AudioUnitElement inElement)
{
    if ((inScope == kAudioUnitScope_Output) && (inElement == 0)) {  // Do we care?
        UInt32 size = 0;
        if (AudioUnitGetPropertyInfo(inUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &size, NULL) ==
            noErr) {
            AudioStreamBasicDescription format;
            if (AudioUnitGetProperty(
                    inUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &format, &size) == noErr) {
                                    ...
                                    AudioBufferList *ioData)
{
    ...
    NFSoundCardDriverInternals *internals = (NFSoundCardDriverInternals *)inRefCon;
    bool silence = !internals->adapter->getFrames((float *)ioData->mBuffers[0].mData,
...
{
    if (dispatch_get_specific(mainQueueKey) == mainQueueKey) {       // Happens on the main thread.
        if (!internals->audioUnitRunning && internals->isPlaying) {  // Our audio should run, but it doesn't do it
...
{
    NFSoundCardDriverInternals *internals = (NFSoundCardDriverInternals *)observer;
...
static void onMediaServerReset(
    CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo)
{
    if (dispatch_get_specific(mainQueueKey) == mainQueueKey) {  // Happens on the main thread.
        NFSoundCardDriverInternals *internals = (NFSoundCardDriverInternals *)observer;
...
static void onAudioSessionInterrupted(
    CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo)
{
    NFSoundCardDriverInternals *internals = (NFSoundCardDriverInternals *)observer;
    NSDictionary *info = (__bridge NSDictionary *)userInfo;
    NSNumber *interruption = [info objectForKey:AVAudioSessionInterruptionTypeKey];
    if (interruption == nil)
        return;
...
#pragma clang diagnostic ignored "-Wtautological-pointer-compare"
            if (&AVAudioSessionInterruptionWasSuspendedKey != NULL) {
                NSNumber *obj = [info objectForKey:AVAudioSessionInterruptionWasSuspendedKey];
...
#pragma clang diagnostic pop
        ...
        case AVAudioSessionInterruptionTypeEnded: {
            NSNumber *shouldResume = [info objectForKey:AVAudioSessionInterruptionOptionKey];
            if ((shouldResume == nil) ||
                [shouldResume unsignedIntegerValue] == AVAudioSessionInterruptionOptionShouldResume)
                endInterruption(internals);
        } break;
    };
}
...
static void setSamplerateAndBuffersize(NFSoundCardDriverInternals *internals)
{
    if (dispatch_get_specific(mainQueueKey) == mainQueueKey) {  // Happens on the main thread.
        if (internals->outputSamplerate > 0) {
            ...
            if ([[AVAudioSession sharedInstance] preferredSampleRate] != internals->outputSamplerate)
                [[AVAudioSession sharedInstance] setPreferredSampleRate:internals->outputSamplerate error:NULL];

            float numFrames = (float)NFDriverAdapter::getOptimalNumberOfFrames(internals->outputSamplerate);
            [[AVAudioSession sharedInstance] setPreferredIOBufferDuration:numFrames / float(internals->outputSamplerate)
                                                                    error:NULL];
        }
    } else
        dispatch_async(dispatch_get_main_queue(),
...
static void recreateAudioUnit(NFSoundCardDriverInternals *internals)
{
    if (dispatch_get_specific(mainQueueKey) == mainQueueKey) {  // Happens on the main thread.
        ...
        if (AudioUnitSetProperty(
                unit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &value, sizeof(value)) != noErr) {
            destroyAudioUnit(&unit);
            internals->errorCallback(internals->clientdata, "Can't enable output IO for the audio unit.", 0);
            return;
        }
        if (AudioUnitAddPropertyListener(
                unit, kAudioUnitProperty_StreamFormat, streamFormatChangedCallback, internals) != noErr) {
            destroyAudioUnit(&unit);
        ...
        UInt32 size = 0;
        if (AudioUnitGetPropertyInfo(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &size, NULL) !=
            noErr) {
            destroyAudioUnit(&unit);
            internals->errorCallback(internals->clientdata, "Can't get the output stream format info size.", 0);
            return;
        }
        AudioStreamBasicDescription format;
        if (AudioUnitGetProperty(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &format, &size) !=
            noErr) {
            destroyAudioUnit(&unit);
        ...
        format.mChannelsPerFrame = 2;  // On iOS we can fix this. On Mac OSX we can not.
        if (AudioUnitSetProperty(
                unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &format, sizeof(format)) != noErr) {
            destroyAudioUnit(&unit);
                                     ...
                                     NF_DID_RENDER_CALLBACK did_render_callback)
{
    ...
    internals->isPlaying = internals->appInBackground = internals->audioUnitRunning = false;
    internals->errorCallback = error_callback;

    internals->adapter = new NFDriverAdapter(
        clientdata, stutter_callback, render_callback, error_callback, will_render_callback, did_render_callback);
    recreateAudioUnit(internals);
    ...
    CFNotificationCenterAddObserver(CFNotificationCenterGetLocalCenter(),
                                    internals,
                                    onMediaServerReset,
                                    (CFStringRef)AVAudioSessionMediaServicesWereResetNotification,
                                    NULL,
                                    CFNotificationSuspensionBehaviorDeliverImmediately);
    CFNotificationCenterAddObserver(CFNotificationCenterGetLocalCenter(),
                                    internals,
                                    onAudioSessionInterrupted,
                                    (CFStringRef)AVAudioSessionInterruptionNotification,
                                    NULL,
                                    CFNotificationSuspensionBehaviorDeliverImmediately);
}
